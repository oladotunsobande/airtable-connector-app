...
export function createCronProcessor(deps: CronProcessorDeps): Processor<CronJobPayload> {
  return async (_job) => {
    ...
    const bases = await baseRepository.findForProcessing(config.pipeline.maxBasesPerRun);
    if (bases.length === 0) {
    ...
    for (const base of bases) {
      ...
      await producer.enqueueBase({ baseId: base.airtableId, baseName: base.name });
    }
